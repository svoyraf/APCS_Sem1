...
class starter {
		...
		*/
		while(true){
			...
			if(choice.equals("file")){
				...
				System.out.println("How many words of text would you like to print?");
				...
				System.out.println("type the word that you want to check!");
				
				String check = sc.nextLine();
				System.out.println();
				
				int checks = checksArray(arr,check);
				
				System.out.println("'" + check + "' is found " + checks + " times in" + textFlie);
			}
	...
	*/
	public static int checksArray(String[] arr, String check){
		...
		for(int i = 0; i < arr.length; i ++){
			...
			else if(arr[i].length()!= check.length){
	...
	*/
	public static String mostCommon(String[] arr){
		...
		return ans;	// Dummy value, delete when needed.
	}
